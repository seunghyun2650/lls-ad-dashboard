...
<div class="sum-row">
  ...
  <div style="text-align:right;flex-shrink:0;">
    ...
    def render_ad_table(df_all):
        ...
        hdr = st.columns(W)
        with hdr[0]:
            st.markdown('<p style="font-size:0.7rem;font-weight:600;color:#a1a1aa;margin:0;padding:0.3rem 0 0.5rem 0;border-bottom:2px solid #18181b;">ON/OFF</p>', unsafe_allow_html=True)
        with hdr[1]:
            st.markdown('<p style="font-size:0.7rem;font-weight:600;color:#a1a1aa;margin:0;padding:0.3rem 0 0.5rem 0;border-bottom:2px solid #18181b;">광고 소재</p>', unsafe_allow_html=True)
        with hdr[2]:
            st.markdown('<p style="font-size:0.7rem;font-weight:600;color:#a1a1aa;margin:0;padding:0.3rem 0 0.5rem 0;border-bottom:2px solid #18181b;">상태</p>', unsafe_allow_html=True)
        ...
        for _, row in df_sorted.iterrows():
            ...
            if thumb_src and thumb_link:
                th_html = f'<a href="{thumb_link}" target="_blank" style="display:block;width:44px;height:44px;flex-shrink:0;border-radius:6px;overflow:hidden;border:1px solid #f0f0f0;"><img src="{thumb_src}" style="width:100%;height:100%;object-fit:cover;"></a>'
            elif thumb_src:
                th_html = f'<div style="width:44px;height:44px;flex-shrink:0;border-radius:6px;overflow:hidden;border:1px solid #f0f0f0;"><img src="{thumb_src}" style="width:100%;height:100%;object-fit:cover;"></div>'
            else:
                icon = "🎬" if row.get("영상여부") else "📷"
                th_html = f'<div style="width:44px;height:44px;flex-shrink:0;border-radius:6px;background:#f4f4f5;display:flex;align-items:center;justify-content:center;font-size:1.1rem;">{icon}</div>'
            ...
            with r[1]:
                st.markdown(f'''<div style="display:flex;align-items:center;gap:0.55rem;padding:0.3rem 0 0.3rem 0;">
  {th_html}
  <div style="min-width:0;">
    <div style="font-size:0.82rem;font-weight:600;color:#18181b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="{row['광고명']}">{row['광고명']}</div>
    <div style="font-size:0.65rem;color:#a1a1aa;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{row['광고세트']}</div>
  </div>
</div>''', unsafe_allow_html=True)

            with r[2]:
                if is_active:
                    st.markdown('<span style="display:inline-block;background:#dcfce7;color:#15803d;border-radius:20px;font-size:0.65rem;font-weight:600;padding:3px 9px;white-space:nowrap;">● 운영중</span>', unsafe_allow_html=True)
                else:
                    st.markdown('<span style="display:inline-block;background:#f4f4f5;color:#71717a;border-radius:20px;font-size:0.65rem;font-weight:600;padding:3px 9px;white-space:nowrap;">⏸ 정지</span>', unsafe_allow_html=True)

            with r[3]:
                st.markdown(f'<span style="font-size:0.88rem;font-weight:700;color:{roas_clr};">{roas_pct}</span>', unsafe_allow_html=True)

            with r[4]:
                st.markdown(f'<span style="font-size:0.82rem;color:#18181b;">{row["비용"]:,.0f}원</span>', unsafe_allow_html=True)

            with r[5]:
                st.markdown(f'<span style="font-size:0.82rem;color:#18181b;">{row["노출수"]:,}</span>', unsafe_allow_html=True)

            with r[6]:
                st.markdown(f'<span style="font-size:0.82rem;color:#18181b;">{int(row["구매전환"])}건</span>', unsafe_allow_html=True)

            with r[7]:
                st.markdown(f'<span style="font-size:0.82rem;color:#18181b;">{spend_per}</span>', unsafe_allow_html=True)

            st.markdown('<div style="height:1px;background:#f0f0f0;"></div>', unsafe_allow_html=True)
